...
于是想到用 **SG90 舵机 + ESP8266 + MQTT** 的方案：舵机用胶带固定在**室内对讲分机的单元门开锁按键**旁，转动拨片就能精准按压到按键（效果和手动按完全一致）；ESP8266 通过家中 WiFi 连接公网 MQTT Broker，手机浏览器打开开门页，点击"开门"按钮就会把指令发过去。
